...
from Calculate_PostProb_Toymodel import Post_prob
from neglogpost_cryobife import neglogpost_cryobife
from Generate_Gaussian_Images_Toymodel import Gaussian_Images
...
Black = np.loadtxt('Black') # Black path in coordinate space.
...
Orange = np.loadtxt('Orange') # Orange path in coordinate space.
Orange = Orange - np.ones((len(Orange),2)) # correction to match python notation.

Grid = np.loadtxt('grid') # Coordinate points representing all the models of the system. 400 models going from the point (1,1) to (20,20).
